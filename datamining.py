...
class RandomForest(QMainWindow):
    ...
    def initUi(self):
        ...
        self.groupBox1Layout.addWidget(self.lblPercentTest, 6, 0)
        self.groupBox1Layout.addWidget(self.txtPercentTest, 6, 1)
    ...
    def update(self):
        ...
        if self.feature8.isChecked():
            if len(self.list_corr_features) == 0:
                self.list_corr_features = ff_happiness[features_list[8]]
            else:
                self.list_corr_features = pd.concat([self.list_corr_features, ff_happiness[features_list[8]]], axis=1)

        if self.feature9.isChecked():
            if len(self.list_corr_features) == 0:
                self.list_corr_features = ff_happiness[features_list[9]]
            else:
                self.list_corr_features = pd.concat([self.list_corr_features, ff_happiness[features_list[9]]], axis=1)

        if self.feature10.isChecked():
            if len(self.list_corr_features) == 0:
                self.list_corr_features = ff_happiness[features_list[10]]
            else:
                self.list_corr_features = pd.concat([self.list_corr_features, ff_happiness[features_list[10]]], axis=1)

        if self.feature11.isChecked():
            if len(self.list_corr_features) == 0:
                self.list_corr_features = ff_happiness[features_list[11]]
            else:
                self.list_corr_features = pd.concat([self.list_corr_features, ff_happiness[features_list[11]]], axis=1)
        ...
        ##  Confusion Matrix
        #::------------------------------------
        class_names1 = ['', 'Not looking for a job change', 'Looking for a job change']
...
class DecisionTree(QMainWindow):
    ...
    def initUi(self):
        ...
        self.groupBox1Layout.addWidget(self.lblPercentTest, 6, 0)
        self.groupBox1Layout.addWidget(self.txtPercentTest, 6, 1)
        self.groupBox1Layout.addWidget(self.lblMaxDepth, 7, 0)
        self.groupBox1Layout.addWidget(self.txtMaxDepth, 7, 1)
        self.groupBox1Layout.addWidget(self.btnExecute, 8, 0)
        self.groupBox1Layout.addWidget(self.btnDTFigure, 8, 1)
    ...
    def update(self):
        ...
        class_names1 = ['', 'Looking for a job change', 'Not looking for a job change']
...
class CorrelationPlot(QMainWindow):
    ...
    def initUi(self):
        ...
        self.feature6 = QCheckBox(features_list[6], self)
        self.feature7 = QCheckBox(features_list[7], self)
        self.feature7 = QCheckBox(features_list[8], self)
        self.feature7 = QCheckBox(features_list[9], self)
        self.feature7 = QCheckBox(features_list[10], self)
        self.feature7 = QCheckBox(features_list[11], self)
    ...
    def update(self):
        ...
        X_1 = ff_happiness["target"]

        list_corr_features = pd.DataFrame(ff_happiness["target"])
        if self.feature0.isChecked():
        ...
        if self.feature8.isChecked():
            list_corr_features = pd.concat([list_corr_features, ff_happiness[features_list[8]]], axis=1)

        if self.feature9.isChecked():
            list_corr_features = pd.concat([list_corr_features, ff_happiness[features_list[9]]], axis=1)

        if self.feature10.isChecked():
            list_corr_features = pd.concat([list_corr_features, ff_happiness[features_list[10]]], axis=1)

        if self.feature11.isChecked():
            list_corr_features = pd.concat([list_corr_features, ff_happiness[features_list[11]]], axis=1)
...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        # Histogram : Histogram about the level of happiness in 2017
        # BarPlot : Presents the correlation between the index of happiness and a feature from the datasets.
        # Heatmap : Correlation plot using all the dims in the datasets
        #::----------------------------------------

        EDA1Button = QAction(QIcon('analysis.png'), 'Histogram', self)
        EDA1Button.setStatusTip('Histogram')
        EDA1Button.triggered.connect(self.EDA1)
        EDAMenu.addAction(EDA1Button)

        EDA2Button = QAction(QIcon('analysis.png'), 'Bar Plot', self)
        EDA2Button.setStatusTip('Bar Plot')
        EDA2Button.triggered.connect(self.EDA2)
        EDAMenu.addAction(EDA2Button)

        EDA4Button = QAction(QIcon('analysis.png'), 'Heat Map', self)
...
def data_happiness():
    ...
    ff_happiness = pd.read_csv('C:\\Users\\Adina Dingankar\\OneDrive\\Desktop\\data_clean.csv')
    features_list = ["enrollee_id", "city", "city_development_index", "gender", "relevent_experience",
                     "enrolled_university", "education_level", "major_discipline", "experience", "company_type",
                     "last_new_job", "training_hours"]
    class_names = ['Looking for a job change', 'Not looking for a job change']
